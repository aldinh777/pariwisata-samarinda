...
<script>
import SimpleMaps from 'src/components/SimpleMaps'

export default {
  name: 'PageDetailWorkingSpace',
  ...
  computed: {
    recomendations () {
      return this.$store.state.workingspace.recomendations
    },
    data () {
  ...
  methods: {
    forward (slug) {
      this.$router.push('/workingspace/' + slug)
    }
  },
  preFetch ({ store, currentRoute }) {
    const slug = currentRoute.params.slug
    return Promise.all([
      store.dispatch('workingspace/getData', slug),
      store.dispatch('workingspace/getRecomendations', slug)
    ])
  }
}
</script>
