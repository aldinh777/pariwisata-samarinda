    ...
    <q-card class="q-mt-lg">
      <q-card-section>
        <div class="text-h6 text-grey-8">
          Kuliner
          <q-btn to="/admin/kuliner/tambah" label="Tambah" class="float-right text-capitalize text-indigo-8 shadow-3" icon="add"/>
        </div>
      </q-card-section>
      <q-card-section class="q-pa-none">
        <q-table :data="data" :columns="columns">
          ...
          <template v-slot:body-cell-Action="props">
            <q-td :props="props">
              <q-btn :to="'/admin/kuliner/update/' + props.row.slug" icon="edit" size="sm" flat dense/>
...
export default {
  ...
  methods: {
    ...
    deleteData () {
      this.$axios.delete('/api/kuliner?slug=' + escape(this.slugHapus), {
        headers: {
  ...
  preFetch ({ ssrContext, store }) {
    ...
    return store.dispatch('kuliner/getLogged', cookies.get('token'))
  }
}
</script>
