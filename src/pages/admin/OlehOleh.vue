    ...
    <q-card class="q-mt-lg">
      <q-card-section>
        <div class="text-h6 text-grey-8">
          Oleh-Oleh
          <q-btn to="/admin/oleholeh/tambah" label="Tambah" class="float-right text-capitalize text-indigo-8 shadow-3" icon="add"/>
        </div>
      </q-card-section>
      <q-card-section class="q-pa-none">
        <q-table :data="data" :columns="columns">
          ...
          <template v-slot:body-cell-Action="props">
            <q-td :props="props">
              <q-btn :to="'/admin/oleholeh/update/' + props.row.slug" icon="edit" size="sm" flat dense/>
...
export default {
  ...
  methods: {
    ...
    deleteData () {
      this.$axios.delete('/api/oleholeh?slug=' + escape(this.slugHapus), {
        headers: {
        ...
        this.$store.dispatch('oleholeh/getList')
      }).catch(err => {
        this.$q.notify('Error')
        console.log(err)
      })
  ...
  preFetch ({ ssrContext, store, redirect }) {
    ...
    return store.dispatch('oleholeh/getLogged', { token, csrf })
  }
}
</script>
