...
export async function getList ({ commit }, limit) {
  ...
  const response = await axios.get('http://localhost:8000/api/kuliner' + limitQuery)
...
export async function getData ({ commit }, id) {
  const response = await axios.get('http://localhost:8000/api/kuliner?id=' + id)
...
export async function getRecomendations ({ commit }, id) {
  const response = await axios.get('http://localhost:8000/api/kuliner?limit=3&exceptId=' + id)
