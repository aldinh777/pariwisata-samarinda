...
export async function getList ({ commit }, limit) {
  ...
  const response = await axios.get('http://' + location.hostname + ':8000/api/kafe' + limitQuery)
...
export async function getData ({ commit }, id) {
  const response = await axios.get('http://' + location.hostname + ':8000/api/kafe?id=' + id)
...
export async function getRecomendations ({ commit }, id) {
  const response = await axios.get('http://' + location.hostname + ':8000/api/kafe?limit=3&exceptId=' + id)
