...
export async function getList ({ commit }, limit) {
  ...
  const response = await axios.get('http://' + location.hostname + ':8000/api/oleholeh' + limitQuery)
...
export async function getData ({ commit }, id) {
  const response = await axios.get('http://' + location.hostname + ':8000/api/oleholeh?id=' + id)
...
export async function getRecomendations ({ commit }, id) {
  const response = await axios.get('http://' + location.hostname + ':8000/api/oleholeh?limit=3&exceptId=' + id)
