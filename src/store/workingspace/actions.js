...
export async function getData ({ commit }, id) {
  const response = await axios.get(api.host + api.endpoint.workingspace + '?id=' + id)
...
export async function getRecomendations ({ commit }, id) {
  const response = await axios.get(api.host + api.endpoint.workingspace + '?limit=3&exceptId=' + id)
