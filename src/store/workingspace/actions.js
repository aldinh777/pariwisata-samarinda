...
export async function getList ({ commit }, limit) {
  ...
  const response = await axios.get('http://localhost:8000/api/workingspace' + limitQuery)
...
export async function getData ({ commit }, id) {
  const response = await axios.get('http://localhost:8000/api/workingspace?id=' + id)
...
export async function getRecomendations ({ commit }, id) {
  const response = await axios.get('http://localhost:8000/api/workingspace?limit=3&exceptId=' + id)
